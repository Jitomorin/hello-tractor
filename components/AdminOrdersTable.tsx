  ...
  return (
    <div className="flex items-center">
      <div className="flex-shrink-0 h-20 w-20">
        <img
          className="h-20 w-20"
          src={row.original.listing.images[0]}
          alt=""
        />
      </div>
      <div className="ml-4">
...
export function ActionCell({ value, setSelectedOrder, setOpenOrder }: any) {
  return (
    <div className="flex items-center">
      <button
        onClick={() => {
          setSelectedOrder(value.row.original);
          setOpenOrder(true);
        }}
...
const AdminOrdersTable = ({ placeholder, orders, router }: any) => {
    ...
    () => [
      {
        Header: "Order ID",
        accessor: (value: any) =>
          `${value.listing.make} ${value.listing.model}`,
        Cell: AvatarCell,
        imgAccessor: (value: any) => `${value.listing.images[0]}`,
        numAccessor: "uid",
      },
      {
        Header: "Payment Details",
        accessor: (value: any) =>
          `Ksh ${formatNumber(Number(value.listing.price))}`,
      },
      {
        Header: "Location",
        accessor: (value: any) =>
          `${value.listing.streetAddress}, ${value.listing.city} ${value.listing.country}`,
      },
      {
        Header: "Created at",
        accessor: (value: any) => {
